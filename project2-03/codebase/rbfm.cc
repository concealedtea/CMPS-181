    ...
    const vector<Attribute> &recordDescriptor, const RID &rid)
{
    ...
    SlotDirectoryRecordEntry recordEntry = getSlotDirectoryRecordEntry(pageData, 
                                                                       rid.slotNum);
    ...
    // Check if using forwarding address (page has been moved)
    if (recordEntry.offset < 0) 
    {
        // delete record
        RID forwardRid;
    ...
    if (compactableData == NULL)
        return RBFM_MALLOC_FAILED;
    memcpy(compactableData, ((char*) pageData + slotHeader.freeSpaceOffset), 
           compactableDataSize);
